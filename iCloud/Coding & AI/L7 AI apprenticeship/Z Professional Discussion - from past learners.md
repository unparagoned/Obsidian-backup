...
[Statistical Methods 2](#_Toc228964432)

[Data Mining 8](#_Toc228964433)

[Supervised Machine Learning 8](#_Toc228964434)

[Neural networks ( architecture, gradient, maths in the nodes, gradient descent, weights) 14](#_Toc228964435)

[Clustering 23](#_Toc228964436)

[Types of Machine Learning 34](#_Toc228964437)

[The Machine Learning Pipeline 38](#_Toc228964438)

[Model Drift 44](#_Toc228964439)

[Clean Drift Monitoring Diagram 47](#_Toc228964440)

[How are PSI and Chi‑Square Test Measured? 49](#_Toc228964441)

[KPIs for Algorithm Accuracy 52](#_Toc228964442)

[Poor Data quality 59](#_Toc228964443)

[select and apply the most effective/appropriate AI and data science techniques to solve complex business problems 62](#_Toc228964444)

[How do you disseminate AI and Data Science practices? 67](#_Toc228964445)

[How do you identify trends in the current AI landscape? 71](#_Toc228964446)

[Common algorithms to know 74](#_Toc228964447)

[Hyperparameter tuning 79](#_Toc228964448)

[Definition of SVM (Support Vector Machine) 81](#_Toc228964449)

[✅ Activation Functions in Neural Networks 83](#_Toc228964450)

[What is Softmax? 86](#_Toc228964451)

[Will AI remove jobs? 88](#_Toc228964452)

[Impact of AI on Industry and Society 91](#_Toc228964453)

[Why Do We Use Unsupervised Learning? 95](#_Toc228964454)
...
- **Descriptive statistics** explain _what the data shows_.
- **Inferential statistics** explain _what the data means for a larger population_.
...
_Null Hypothesis_: Test proportions are same for external sharing in September

_Alternative Hypothesis_: Test proportions are different for external sharing in September
...
_Find overall Sample proportions:_

P=(63+44)/(63+44+717+2161)= 0.0358=3.6%

_Test Statics formula:_

The test statics formula is shown in Figure 3: Test Statics formula. This formula is provided in the 2 Proportion Z-tests. I am using this formula as it is to proceed.

![[620e3f451ee2f921444204a36b636868_MD5.png]]
...
_2 Proportion Z-tests:_

Confidence level table is shown below

![[64192db8f9166f3eacaf9a18b2374900_MD5.png]]
...
![[dce10e3a8e4a98dcbf05dc0111eea2cb_MD5.png]]

Figure 5: 2 Proportion Z-test bell curve

The 2 Proportion Z-test shows that Null Hypothesis is correct. Both SharePoint and OneDrive external sharing is comparable for the month of September. Z from calculation is 0.00032 and the Z score from table is 1.96 which is greater than the Z from calculation. Figure 5: 2 Proportion Z-test bell curve shows the position of the comparison point on the bell curve.
...
![[ef2b548d448f7c3743cca18e97609692_MD5.png]]
...
![[6ec540c7a4433e973b4fbcb1c9115bee_MD5.png]]
...
![[9b0da3b99aca85140fb04cfbc408f82c_MD5.png]]
...
- If a neuron's weights make always,
- Gradient becomes **zero**
- Neuron stops learning

**Variants of ReLU (Briefly)**

- **Leaky ReLU:** Allows small gradient when
...
| Formula        |                    |               |
| Output Range   | (0, 1)             | \[0, ∞)       |
| Gradient Issue | Vanishing gradient | Zero for      |
| Speed          | Slower             | Faster        |
...
**Pros:** Fast, simple  
**Cons:** Need to pre‑define , sensitive to outliers
...
- **Clustering:** Finds groups with _no labels_
- **Classification:** Assigns labels using _training data_
...
**Example:**  
Choose to group incidents into 3 patterns.
...
![[c996bbdf77788424d1252b28568a5f00_MD5.png]]
...
- Confirm drift type (data vs concept)
- Collect fresh labelled data
- Retrain model
...
![[96bbac5425ac0eab739c8d8218e304b5_MD5.png]]
...
![[94b8e1abcc43d860b7aa3f31121b9304_MD5.png]]

**Swimlanes included:**

- **Data drift (inputs):** Training baseline vs production feature distributions (PSI/KS/χ²)
- **Prediction drift (outputs):** Prediction score/class mix changes (useful when labels are delayed)
- **Concept drift / Performance:** Ground truth labels → metrics trend (RMSE/F1/AUC) → retrain/redeploy
...
\\(SSR\\) (also known as \\(SSE\\)) measures the variation that the model _cannot_ explain-the distance between actual and predicted values.  
...
When **data quality is not good**, **pure accuracy metrics are unreliable**. In that situation, you should shift focus from _"how accurate is the prediction?"_ to _"how reliable, stable, and trustworthy is the model given bad data?"_
...
📌 _Very interview‑friendly answer for poor data quality_
...
- Convert the problem into a measurable objective:
  - _Reduce cost? Improve SLA compliance? Detect risk earlier? Increase conversion?_
...
- **Forecasting / numerical estimate** → _Regression_ (e.g., time-to-resolve, demand)
- **Yes/No decision** → _Binary classification_ (e.g., churn, fraud)
- **Multiple categories** → _Multi-class classification_ (e.g., ticket routing)
- **Group similar items** → _Clustering_ (e.g., incident patterns)
- **Find unusual behaviour** → _Anomaly detection_ (e.g., security, outages)
- **Recommend next best action** → _Recommenders_
- **Optimise sequential decisions** → _Reinforcement learning_ (less common in enterprises)
...
- Remove those below **minimum support**
- Generate **2‑itemsets**
- Repeat (Apriori property: subset must be frequent)
...
- **Train model on different combinations**
...
Activation functions convert the neuron output into a **non‑linear output**, enabling neural networks to learn **complex patterns**.
...
- **Dying ReLU** (neuron outputs always 0 if )
...
- Apply **exponential** to each input → ensures positivity
- Normalize by dividing by the sum → gives a **probability distribution**
