...
**Word count:** body ≈ 5,495 of 5,500 permitted (5,000 +10%). References, appendices and this header are excluded. There is effectively no headroom — anything added needs something cut. Sections 5 (≈875) and 3 (≈690) are the largest and the first place to look if you need room.

**Seven items need confirming before submission — see "Working notes" at the end of this file.**
...
**Embedding comparison.** Independently of the classifier, silhouette scores on a 50,000-record sample measured how cleanly each vectorisation separates concepts. An initial run included a word-plus-character n-gram combination whose parameters had been grid-searched on the same sample it was then scored against. It topped the table, but that is not a fair comparison — the search had optimised directly against the evaluation data, so the margin reflects tuning rather than a better representation. I excluded it and reran over the untuned vectorisers, on which the mpnet sentence transformer scored highest, ahead of TF-IDF and TF word n-grams and E5-large.

That mpnet separates the classes best is expected: dense embeddings capture semantic similarity between differently worded descriptions of one concept, which n-grams cannot. It did not translate into a materially better classifier. Downstream, mpnet was *statistically* better than word-only TF-IDF but by only about 0.003 macro-F1, at roughly 76× the runtime. Simple word n-grams were therefore selected: statistical significance is not material significance, and the simpler representation is faster, easier to maintain and easier to interpret.
...
**LinearSVC optimisation.** HalvingRandomSearchCV narrowed the field to LinearSVC, SVC with a linear kernel, and PassiveAggressiveClassifier; paired t-tests showed LinearSVC significantly better than both at the 95% level. Plotting hyperparameters against score and runtime then guided the refined ranges — a two-dimensional plot with runtime encoded by colour showed `min_df=1` occupying clusters both faster and better scoring than `min_df=2`, which tabulated results do not reveal. The final configuration was TF-IDF (1–3 word n-grams, `min_df=1`, l2 norm) with LinearSVC (`penalty='l1'`, `C=2.8`, squared hinge loss, `dual=False`, `class_weight='balanced'`, `max_iter=10,000`); several values of C were statistically indistinguishable, so the lower end of that range was taken to reduce overfitting risk. The l1 penalty performs implicit feature selection, producing a sparser weight matrix and faster inference. Confidence intervals were computed by bootstrap. Of the training populations tested, 10% sqrt-weighted gave 1.3pp better macro-F1 but marginally lower accuracy than plain 10%, while the full population was better on both and gives the simplest pipeline, so it was used with balanced class weighting.

**Neural network and transformer optimisation.** Optuna's automated search and hyperparameter-importance visualisations were a clear improvement on the manual approach used with scikit-learn, and I intend to use it there in future. Architecture type was the most important factor for F1, batch size for training time; full configurations are at Appendix C. For imbalance, sqrt-weighted data with an unweighted CNN (0.776) beat both fully balanced weighting (0.721) and the unweighted baseline (0.757). Among transformers, SEC-BERT led on the 1% subset (0.754), consistent with its financial pre-training, and reached 0.782 on the full population — but required 14.3 hours of GPU training, where the 10% sqrt-weighted configuration reached 0.779 roughly 14× faster.
...
**Deployment.** The model was integrated into the R production pipeline via `reticulate`, with outputs ingested to Oracle. Working with DevOps, on-demand compute starts an EC2 instance for the daily batch and shuts it down on completion — cheaper than permanent provisioning and more available than contended GPU capacity.
...
Supervised ML extends taxonomy classification from 30% to close to full coverage and meets every defined KPI, and the gains came disproportionately from data preparation and label design rather than model architecture — the most transferable lesson from the project.

Selecting LinearSVC over transformers shows why solutions must be evaluated against the full operational requirement rather than one metric: SEC-BERT's marginally lower 0.782 comes with GPU infrastructure, 14 hours of training, slower inference and a more complex maintenance profile. Explainability was likewise a first-order requirement, not a nicety — interpretable feature weights are what made error analysis, stakeholder communication and the analyst dashboard possible, and a marginally better black box would have carried a governance risk the business could not accept. Other decisions deliberately sacrificed raw performance for usability: a bespoke model per taxonomy would score best but would give analysts inconsistent class names, so one main taxonomy per document type is applied universally. That trade-off came from asking SMEs why predictions that looked right were scored wrong — SME questioning reshaping the technical design rather than merely validating it.

Communication required deliberate adaptation, and my approach changed in response to how stakeholders reacted. Early explanations were too detailed for some audiences, so I moved to visual, example-led ones: simple decision trees showing what was split on, a two-dimensional SVM decision boundary, and a worked example conveying the difference between weighted and macro scores. With analysts I focused on outcomes — confusion matrices for good and poor classes, concrete examples of the mistakes to expect — and requested feedback at each step; understanding grew and use of the ML category in profiling increased. With technical stakeholders I shifted from stating what I had done to explaining why, linking theory to experimental results. With DevOps the focus was benchmarks, memory and the cost-benefit of specific EC2 instance types. With managers it was outcomes, timescales, resourcing and blockers they could remove: memos with cost-benefit analysis secured additional infrastructure funding, and framing the case around delivery capacity got a further developer allocated. I now recommend a standard structure for technical communications here — headline figures first, plain-English explanation with illustrations, technical detail in an appendix. Documentation was treated as a deliverable in its own right: the README and setup script let many analysts install and run the tool themselves, scaling adoption and freeing me to develop.

**Recommendations**

1. Extend the pipeline to further document types, revalidating taxonomy, extraction and PII assumptions for each rather than assuming transfer.
2. Operate the drift monitoring already defined: watch inputs for new or changed taxonomies, and treat a 2pp drop in accuracy or macro-F1 with non-overlapping confidence intervals over two consecutive days as the retraining trigger. Retain model, data and preprocessing versions so rollback is always available.
3. Establish formal data contracts with the downstream teams now consuming Hubble output, since informal dependencies on a widely relied-upon source are a continuity risk.
4. Explore targeted active learning for the weakest classes, using analyst feedback on uncertain predictions.
...
6. Move the test suite into a GitLab CI pipeline, and evaluate porting the R components to Python.

---

## 10. Summary of Findings

A supervised classifier was developed and deployed to categorise untagged items in iXBRL financial documents. The selected pipeline — TF-IDF word n-grams with LinearSVC and balanced class weights — achieved macro-F1 0.787 and accuracy 97.1%, exceeding the 0.70 KPI, at 2.7μs per record on existing CPU infrastructure. It was chosen over CNN and transformer alternatives on a weighted decision matrix combining performance, interpretability, efficiency and fit with HMRC's infrastructure and governance requirements, and outperformed SEC-BERT on robustness testing as well as headline metrics. Data preparation contributed more than model selection: canonicalisation alone moved macro-F1 from below 0.50 to above 0.70. The project followed CRISP-DM under Kanban management and is in production, adopted by multiple analyst teams.

---

## 11. Implications

**Analytical capability.** Teams previously limited to the tagged minority can now analyse the full population, and receive it within days rather than after the taxonomy update cycle — both of which change what is achievable inside the twelve-month enquiry window. This improves the statistical basis for policy advice and the completeness of risk identification; Hubble has supported projects that identified hundreds of billions of pounds of incorrectly stated figures in returns.
...
**Reusable patterns.** The project establishes a methodology for applying supervised ML to semi-structured tax data — existing tags as supervision, iterative CRISP-DM development, sub-population correlation validation to make experimentation affordable, sqrt-weighted sampling for imbalance, weighted decision matrices for selection — and the on-demand compute pattern is a template for other periodic high-volume workloads.
...
**Evaluation is on tagged data; the use case is untagged data.** This is the most significant limitation. The model is trained and evaluated entirely on items the preparer chose to tag, but its purpose is to classify items they did not. If the two populations differ systematically — for instance where an item was left untagged precisely because no suitable concept exists — measured performance will overstate real-world performance. The ideal mitigation is a human-labelled sample of untagged items, but that requires tax-trained SMEs committed to higher-priority work. In the interim the mitigations are monitoring prediction distributions in production, analyst feedback, and the human-in-the-loop control.
