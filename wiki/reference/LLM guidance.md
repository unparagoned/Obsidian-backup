...
tags:
  ...
  - ai
type: reference
source: raw/notes/coding-ai/LLM guidance.md
author: user
---

**Summary:** The user's working prompt library and agent-workflow rules: a Claude-oriented workflow (plan mode, subagents, lessons file, verification), OpenAI's goal/format/warnings/context structure, a production system-prompt template, a JSON meta-prompt for writing prompts, XML-vs-markdown structuring, and a list of key techniques.

This is a **reference** page — procedures the user follows, not graded evidence. Content copied as written with light cleanup; `raw/notes/coding-ai/Prompting.md` merged in as its own section.

Related: [[Claude Code]] · [[Compare LLM]] · [[LLM - how they work]] · [[AI tools and learning sources]]
...
# Prompting (video notes)

Merged from `raw/notes/coding-ai/Prompting.md`.
...
- Add more context
- Output requirements
- Can tell it the steps to take in terms of thinking, but thinking modes have it already enabled

Could have different parts, almost like different agents, but just in a prompt. e.g. Stage one make a draft. Stage two critically review the draft…

# TODO

- The "2026 testing shows models attend to <tag> structures 15% better" claim comes from a Reddit post that also advertises two products; treat as marketing until a real evaluation is found.
- The `#TODO look` on the nanonets structured-outputs cookbook is still open.
- Model-specific advice ages fast; date the sections.
